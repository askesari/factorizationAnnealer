...
class IntegerFactorisation:
    ...
    @classmethod
    def get_var_components_dict(cls, expr):
        """
        this class method returns the variables used in the input expression
        broken down based on the variable type i.e. p, q, z, y, etc. in the form of
        a dictionary
        """
        var_comp = dict()
        for v in list(expr.free_symbols):
            if v.func == Indexed:
                key = v.args[0]
                if var_comp.get(key) is not None:
                    var_comp[key].append(v.args[1])
                else:
                    var_comp[key] = list()
                    var_comp[key].append(v.args[1])
